import { Alert, Platform, Linking } from 'react-native';
import FileViewer from 'react-native-file-viewer';
import RNFS from 'react-native-fs';
import { generatePdf } from './generatePdf';
import { showSuccessToast, showErrorToast, showInfoToast } from './toastUtils';

const walstarLogoBase64 =
  'data:image/png;base64,';

export const handleDownloadPdf = async (generateReportTable, reportTitle) => {
  try {
    // Create the HTML content dynamically
    const htmlContent = `
     <html>
  <head>
    <style>
      body {
        font-family: Arial;
        margin: 20px;
        padding-top: 30px; /* Space for logo */
      }
      h1 {
        color: #1c2f87;
        text-align: center;
      }
      table {
        width: 100%;
        border-collapse: collapse;
        margin-top: 20px;
      }
      th {
        background-color: #1c2f87;
        color: white;
        padding: 10px;
        text-align: left;
      }
      td {
        padding: 8px;
        border-bottom: 1px solid #ddd;
      }
      .total-row {
        font-weight: bold;
      }
      /* Add style for logo */
      .logo {
        position: absolute;
        top: 20px;
        left: 20px;
        width: 100px; /* Adjust the size of the logo */
        height: auto;
      }
    </style>
  </head>
  <body>
    <!-- Logo image at the top left corner -->
    <img src="${walstarLogoBase64}" class="logo" alt="Logo" />
    
   <h1>${reportTitle}</h1>
    ${generateReportTable()} <!-- Extract table generation to a function -->
    <p style="margin-top: 30px; font-size: 12px; color: #666;">
      Generated on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}
    </p>
  </body>
</html>
    `;

    // Generate the PDF file
    const pdfPath = await generatePdf(
      htmlContent,
      `${reportTitle.replace(/[^a-z0-9]/gi, '_')}_${
        new Date().toISOString().split('T')[0]
      }.pdf`,
    );

    console.log("pdfPath ------------->",  pdfPath);

    // On Android, try to copy/move the file to public directories
    let finalPath = pdfPath;
    let fileAccessible = true;
    
    if (Platform.OS === 'android') {
      try {
        const fileName = pdfPath.split('/').pop();
        
        // Try Downloads directory first (better for Android 15)
        const downloadsDir = RNFS.DownloadDirectoryPath || `${RNFS.ExternalStorageDirectoryPath}/Download`;
        
        if (!(await RNFS.exists(downloadsDir))) {
          await RNFS.mkdir(downloadsDir);
        }
        
        const destPath = `${downloadsDir}/${fileName}`;
        if (await RNFS.exists(destPath)) {
          await RNFS.unlink(destPath);
        }
        
        await RNFS.copyFile(pdfPath, destPath);
        finalPath = destPath;
        
        console.log('PDF copied to Downloads:', destPath);
      } catch (err) {
        console.warn('Moving to Downloads failed, attempting Documents:', err);
        try {
          const fileName = pdfPath.split('/').pop();
          const publicDocuments = `${RNFS.ExternalStorageDirectoryPath}/Documents`;
          const hasDocs = await RNFS.exists(publicDocuments);
          if (!hasDocs) {
            await RNFS.mkdir(publicDocuments);
          }
          const destPath = `${publicDocuments}/${fileName}`;
          if (await RNFS.exists(destPath)) {
            await RNFS.unlink(destPath);
          }
          await RNFS.copyFile(pdfPath, destPath);
          finalPath = destPath;
          console.log('PDF copied to Documents:', destPath);
        } catch (fallbackErr) {
          console.warn('Fallback to Documents failed; using app-scoped path:', fallbackErr);
          finalPath = pdfPath;
          fileAccessible = false; // File is in app-scoped storage
        }
      }
    }
    
    // Show themed success message
    const successMessage = fileAccessible 
      ? 'Report saved successfully to Downloads folder'
      : 'Report saved to app storage (due to permission restrictions)';
    
    // Show success toast with app theme colors
    showSuccessToast('PDF Generated Successfully', successMessage);

    // Show custom themed alert after a short delay
    setTimeout(() => {
      Alert.alert(
        '📄 PDF Report Ready',
        `Your ${reportTitle} has been generated successfully!`,
        [
          {
            text: 'Close',
            style: 'default',
          },
          {
            text: 'View PDF',
            style: 'default',
            onPress: async () => {
              try {
                // First try with FileViewer
                await FileViewer.open(finalPath, {
                  showOpenWithDialog: true,
                  onDismiss: () => console.log('PDF viewer dismissed'),
                  // Hint the MIME type on Android
                  contentType: Platform.OS === 'android' ? 'application/pdf' : undefined,
                });
              } catch (err) {
                console.error('Failed to open PDF with FileViewer:', err);
                
                // Fallback: Try to open with system default app using Linking
                try {
                  const fileUri = Platform.OS === 'android' 
                    ? `file://${finalPath}` 
                    : finalPath;
                  
                  console.log('Trying to open with Linking:', fileUri);
                  
                  const canOpen = await Linking.canOpenURL(fileUri);
                  if (canOpen) {
                    await Linking.openURL(fileUri);
                  } else {
                    throw new Error('Cannot open file with system apps');
                  }
                } catch (linkingErr) {
                  console.error('Failed to open PDF with Linking:', linkingErr);
                  
                  const msg = String(err?.message || err);
                  if (Platform.OS === 'android' && (
                    msg.includes('No app associated') || 
                    msg.includes('No Activity found') ||
                    msg.includes('Unable to resolve activity')
                  )) {
                    Alert.alert(
                      '📱 No PDF Viewer Found',
                      'Please install a PDF viewer app (Google PDF Viewer, Adobe Acrobat) to view the report.',
                      [
                        { text: 'OK', style: 'default' },
                        {
                          text: 'Open Downloads',
                          onPress: () => {
                            // Try to open Downloads folder
                            Linking.openURL(`content://com.android.externalstorage.documents/root/primary:Download`).catch(() => {
                              showInfoToast('Manual Access Required', 'Please navigate to Downloads folder manually to view the PDF.');
                            });
                          }
                        }
                      ]
                    );
                  } else {
                    Alert.alert(
                      '📂 Access Your Report', 
                      'You can find your PDF report in the Downloads folder.',
                      [
                        { text: 'OK', style: 'default' },
                        {
                          text: 'Open Downloads',
                          onPress: () => {
                            Linking.openURL(`content://com.android.externalstorage.documents/root/primary:Download`).catch(() => {
                              showInfoToast('Manual Access Required', 'Please navigate to Downloads folder manually to view the PDF.');
                            });
                          }
                        }
                      ]
                    );
                  }
                }
              }
            },
          },
        ]
      );
    }, 1500); // Show alert after toast
  } catch (error) {
    console.error('PDF generation failed:', error);
    
    // Show error toast first
    showErrorToast('PDF Generation Failed', 'Unable to generate PDF report. Please try again.');
    
    // Show detailed error alert
    setTimeout(() => {
      Alert.alert(
        '❌ Generation Error',
        `Failed to generate ${reportTitle}.\n\n${error.message || 'Please try again later.'}`,
        [
          { text: 'OK', style: 'default' }
        ]
      );
    }, 1000);
  }
};