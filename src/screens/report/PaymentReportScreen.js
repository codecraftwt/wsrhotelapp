...
const PaymentReportScreen = () => {
  ...
  useEffect(() => {
    console.log(
      '🚀 Initial load - fetching payment reports without date filters',
    );
      ...
      return (
        <View style={styles.loadingMoreContainer}>
          <Text style={styles.loadingMoreText}>
            No more reports to load ({paymentReports.length} total)
          </Text>
        </View>
      );
    }
  ...
  const renderTableRow = ({ item, index }) => (
    <View style={styles.tableRow}>
      <Text style={styles.tableCell}>{item?.records?.[0]?.date || '-'}</Text>
      ...
      {viewMode === 'card' ? (
        <FlatList
          ...
          keyExtractor={(item, index) =>
            `payment_${item?.id || index}_${index}`
          }
        ...
        <ScrollView horizontal>
          <View style={styles.tableWrapper}>
            ...
            <FlatList
              data={paymentReports}
              keyExtractor={(item, index) =>
                `payment_table_${item?.id || index}_${index}`
              }
